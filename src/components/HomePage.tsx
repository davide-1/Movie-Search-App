...
type HomePageProps = { onSearch: (q: string) => void };

export default function HomePage({ onSearch }: HomePageProps) {
  const { favorites } = useFavorites();

  const { data: trendingData, fetchNextPage: fetchTrending, hasNextPage: hasMoreTrending } =
    useInfiniteMovies("trending");
  const { data: popularData, fetchNextPage: fetchPopular, hasNextPage: hasMorePopular } =
    useInfiniteMovies("popular");
  const { data: upcomingData, fetchNextPage: fetchUpcoming, hasNextPage: hasMoreUpcoming } =
    useInfiniteMovies("upcoming");
  ...
  return (
    <>
      {/* Search bar stays visible, but redirects to /search */}
