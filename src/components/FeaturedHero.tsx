...
import { useFeaturedMovie, useFeaturedTrailer } from "../hooks/useFeaturedMovie";
...
export default function FeaturedHero() {
  ...
  const toDetails = movie.media_type === "tv" ? `/tv/${movie.id}` : `/movie/${movie.id}`;
...
>
  <Box
    ...
    borderRadius: 3,
    overflow: "hidden",
    bgcolor: "grey.800",                              // lighter grey like Netflix
    boxShadow: "0 12px 40px rgba(0,0,0,.35)",        // soft drop shadow
  ...
  >
    ...
    <Stack
      ...
      spacing={1}
      sx={{ position: "relative", zIndex: 2, height: "100%", p: { xs: 2, sm: 3 }, maxWidth: 600 }}
    >
      <Typography component="h1" sx={{ fontWeight: 700, fontSize: { xs: 18, sm: 22, md: 28 } }}>
        {title}
      </Typography>
      <Typography
        variant="body2"
      ...
      <Stack direction="row" spacing={1}>
        <Button size="small" variant="contained" onClick={() => navigate(toDetails)}>
          More Info
        </Button>
        <Button size="small" variant="outlined" onClick={() => navigate("/favorites")}>
          ❤️ Favorites
        </Button>
      </Stack>
    </Stack>
